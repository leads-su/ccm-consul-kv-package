...
abstract class AbstractService
{
    ...
    private function initializeClient(): void
    {
        ...
        $this->client = Http::baseUrl(sprintf(
            ...
            'X-Consul-Token'    =>  Arr::get($activeConnection, 'access_token') ?? config('consul.kv.access_token', ''),
        ])->timeout(config('consul.kv.timeout', 15));
    }
    ...
    {
        $connectionName = config('consul.kv.default', 'default');
        $connectionsList = config('consul.kv.connections', [
            'default'       =>  [
                'scheme'    =>  'http',
                'host'      =>  '127.0.0.1',
                'port'      =>  8500,
        ...
        $withAutoSelect = config('consul.kv.auto_select', false);
        $withRandomServer = config('consul.kv.use_random', false);
